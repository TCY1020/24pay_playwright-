...
- `PAYMENT_STATS_PAGE`（24pay 代收訂單統計選單與 tab/iframe 對應 id）
- `NOTIFY_24PAY_SCHEDULED_REPORT_USER_ID`（定時報表訊息開頭 @ 通知對象，可字串或陣列）
...
- 啟動時會先導頁到代收訂單統計頁（`PAYMENT_STATS_PAGE.mainMenuId/subMenuId`）。
- 依 `REPORT_HOURS_UTC8` 用 UTC+8 計算下一個執行點，`setTimeout` 單次排程，執行完再遞迴排下一次。
- 每次執行會先切回代收統計 tab，將查詢條件填為「當日 00:00:00 ~ 23:59:59」，再送出查詢。
- 查詢後會點開下拉箭頭，再從指定 iframe 讀主表與明細表資料。
- 報表內容以「各商户汇总」列（找不到則退回第一列）計算總跑量，並取前五商戶 `OrderSuccessAmount`。
- Telegram 訊息格式：`@通知對象` + `MM/DD HH:mm總跑量 xxxxx.xx` + `前五家` 清單；送到 `BALANCE_NOTIFICATION_GROUP_CHAT_ID`。
